...
struct Strategy{
  ...
  // 22 is special, as it is a bust for player but a push for dealer.
  // values >= 23 can be grouped, as they are all bust.
  ...
  Strategy(Prob<int>& prob) : prob(prob) {
    ...
    //   return;
    // else{
    endVals = {16, 17, 18, 19, 20, 21, 22, 23};
      bootstrapDealer();
      bootstrapPlayer();
  ...
  void bootstrapPlayer(){
    ...
    for (int i=1; i<11; i++){ // player busts on 22 and 23
      playerTable.add(StatePair(HandState(22, false, false), i), StrategyElement({-1, -999, -999, -999}));
      playerTable.add(StatePair(HandState(23, false, false), i), StrategyElement({-1, -999, -999, -999}));
    }

    for (int i=21; i>10; i--){ // 21H to 11H
      for (int j=1; j<11; j++){
	StatePair sPair = StatePair(HandState(i, false, false), j);
	...
	StatePair sPair = StatePair(HandState(i, false, false), j);
	playerTable.add(sPair, computePlayerAction(sPair));
	sPair = StatePair(HandState(i, true, false), j);
	playerTable.add(sPair, computePlayerAction(sPair));
      }
    }
    ...
    for (int i=4; i>2; i--){ // 4S and 3S
      for (int j=1; j<11; j++){
    	StatePair sPair = StatePair(HandState(i, true, false), j);
    	playerTable.add(sPair, computePlayerAction(sPair));
      }
    }
    ...
    for (int i=10; i>1; i--){ // TT through 22
      for (int j=1; j<11; j++){
    	StatePair sPair = StatePair(HandState(2*i, false, true), j);
    	playerTable.add(sPair, computePlayerAction(sPair));
      }
    }

    for (int j=1; j<11; j++){ // AA
      StatePair sPair = StatePair(HandState(2, true, true), j); 
      playerTable.add(sPair, computePlayerAction(sPair));
    }
  ...
  StrategyElement computePlayerAction(StatePair sp){
    ...
    Prob<int> pfs = Prob<int>(endVals, {0., 0., 0., 0., 0., 0., 0., 0.}); // player's final pdf. Will be different for each action.

    // stand
    int newVal = sp.hs.value;
    if (sp.hs.soft && newVal + 10 <= 21)
      newVal += 10;
    if      (newVal <= 16) pfs.pdf = {1., 0., 0., 0., 0., 0., 0., 0.};
    else if (newVal == 17) pfs.pdf = {0., 1., 0., 0., 0., 0., 0., 0.};
    else if (newVal == 18) pfs.pdf = {0., 0., 1., 0., 0., 0., 0., 0.};
    else if (newVal == 19) pfs.pdf = {0., 0., 0., 1., 0., 0., 0., 0.};
    else if (newVal == 20) pfs.pdf = {0., 0., 0., 0., 1., 0., 0., 0.};
    else if (newVal == 21) pfs.pdf = {0., 0., 0., 0., 0., 1., 0., 0.};
    else if (newVal == 22) pfs.pdf = {0., 0., 0., 0., 0., 0., 1., 0.};
    else                   pfs.pdf = {0., 0., 0., 0., 0., 0., 0., 1.};
    ...
    for (int i=0; i<10; i++){
      if ((sp.hs.soft || i == 0) && sp.hs.value < 22){ // player has an ace already or gets an ace
    	  if (sp.hs.value + i + 1 <= 10)
    	    ev += prob.pdf[i] * playerTable.get(StatePair(HandState(sp.hs.value+i+1, true, false), sp.dealerUpcard)).bestValue;
    	  else if (sp.hs.value + i + 1 == 11)
    	    ev += prob.pdf[i] * playerTable.get(StatePair(HandState(21, false, false), sp.dealerUpcard)).bestValue;
    	  else
    	    ev += prob.pdf[i] * playerTable.get(StatePair(HandState(sp.hs.value+i+1,false, false), sp.dealerUpcard)).bestValue;
      }
      else{
    	  if (sp.hs.value+i+1 < 22)
    	    ev += prob.pdf[i] * playerTable.get(StatePair(HandState(sp.hs.value+1+i, false, false),sp.dealerUpcard)).bestValue;
    	  else
    	    ev += prob.pdf[i] * playerTable.get(StatePair(HandState(22, false, false), sp.dealerUpcard)).bestValue; // =-1 !
      }
    }
    ...
    for (int i=0; i<10; i++){
      ...
      if (newVal <= 16)      pfs += prob.pdf[i] * Prob<int>(endVals, {1., 0., 0., 0., 0., 0., 0., 0.});
      else if (newVal == 17) pfs += prob.pdf[i] * Prob<int>(endVals, {0., 1., 0., 0., 0., 0., 0., 0.});
      else if (newVal == 18) pfs += prob.pdf[i] * Prob<int>(endVals, {0., 0., 1., 0., 0., 0., 0., 0.});
      else if (newVal == 19) pfs += prob.pdf[i] * Prob<int>(endVals, {0., 0., 0., 1., 0., 0., 0., 0.});
      else if (newVal == 20) pfs += prob.pdf[i] * Prob<int>(endVals, {0., 0., 0., 0., 1., 0., 0., 0.});
      else if (newVal == 21) pfs += prob.pdf[i] * Prob<int>(endVals, {0., 0., 0., 0., 0., 1., 0., 0.});
      else if (newVal == 22) pfs += prob.pdf[i] * Prob<int>(endVals, {0., 0., 0., 0., 0., 0., 1., 0.});
      else                   pfs += prob.pdf[i] * Prob<int>(endVals, {0., 0., 0., 0., 0., 0., 0., 1.});
    }
      ...
      if (splitVal == 1){ //splitting aces
    	for (int i=0; i<10; i++){
    	  if (i == 9) // A + T
    	    ...
    	    ev += prob.pdf[i] * playerTable.get(StatePair(HandState(splitVal+i+1, true, false), sp.dealerUpcard)).bestValue;
    	}
      }
    	...
    	for (int i=0; i<10; i++){
    	  if (i == 0){
    	    if (splitVal + 1 == 11) // T + A
    	      ...
    	      ev += prob.pdf[i] * playerTable.get(StatePair(HandState(splitVal+1, true, false), sp.dealerUpcard)).bestValue;
    	  }
    	  else if (splitVal == i + 1) // X + X again
    	      ; // deal with this by adjusting ev
    	  else
    	    ev += prob.pdf[i] * playerTable.get(StatePair(HandState(splitVal+i+1, false, false), sp.dealerUpcard)).bestValue;
    	}
      }
  ...
  double computeExpectation(vector<double> playerPdf, vector<double> dealerPdf){
    ...
    // if player busts he loses regardless of dealer: i.e. states 22 and 23
    result -= playerPdf[6];
    result -= playerPdf[7];
    
    // non-busted player wins if dealer busts @ 23 or righer or his value is higher than dealer's
    // he pushes if the values are the same or dealer busts at 22
    // and he loses if his value is less than dealer's
    for (int i=0; i<6; i++){ // start at 16, proceed to 21

      result += playerPdf[i] * dealerPdf[7]; // player wins if dealer busts @ 23
  ...
  // Dealer has only 7 possible final states: 17, 18, 19, 20, 21, 22 and >= 23, which we represent as 23.
  ...
  Prob<int> computeDealerFinalState(int upcard){
    Prob<int> result = Prob<int>(endVals, {0., 0., 0., 0., 0., 0., 0., 0.});
    ...
    if (upcard == 1){
      ...
      for (int i=0; i<9; i++){
	if (upcard + i + 1 <= 6)
	  result += condFactor * prob.pdf[i] * dealerTable.get(HandState(upcard+i+1, true, false));
	else
	  result += condFactor * prob.pdf[i] * dealerTable.get(HandState(upcard+i+1+10, false, false));
    ...
    // hence mult probs by 1/(1-prob[0])
      ...
      for (int i=1; i<10; i++){
	result += condFactor * prob.pdf[i] * dealerTable.get(HandState(upcard+1+i, false, false));
      }
    }
    ...
    // if his upcard is 2-9, his second card can be anything
      ...
      if ((upcard + 1) <= 6)
	result += prob.pdf[0] * dealerTable.get(HandState(upcard+1, true, false));
      else 
	result += prob.pdf[0] * dealerTable.get(HandState(upcard+1+10, false, false));
      // note the third case, (upcard + 1) >= 12, is not possible!
      
      // then 2-10
      for (int i=1; i<10; i++)
	result += prob.pdf[i] * dealerTable.get(HandState(upcard+1+i, false, false));
    }
  ...
  void computeDealerTable(){
    ...
    dealerTable.add(HandState(17, false, false), Prob<int>(endVals, {0., 1., 0., 0., 0., 0., 0., 0.}));
    dealerTable.add(HandState(18, false, false), Prob<int>(endVals, {0., 0., 1., 0., 0., 0., 0., 0.}));
    dealerTable.add(HandState(19, false, false), Prob<int>(endVals, {0., 0., 0., 1., 0., 0., 0., 0.}));
    dealerTable.add(HandState(20, false, false), Prob<int>(endVals, {0., 0., 0., 0., 1., 0., 0., 0.}));
    dealerTable.add(HandState(21, false, false), Prob<int>(endVals, {0., 0., 0., 0., 0., 1., 0., 0.}));
    dealerTable.add(HandState(22, false, false), Prob<int>(endVals, {0., 0., 0., 0., 0., 0., 1., 0.}));
    dealerTable.add(HandState(23, false, false), Prob<int>(endVals, {0., 0., 0., 0., 0., 0., 0., 1.}));
    ...
    HandState hs;
    for (int i=16; i>10; i--){
      hs = HandState(i, false, false);
      dealerTable.add(hs, computeDealerState(hs));
    }
    for (int i=10; i>3; i--){
      hs = HandState(i, false, false);
      dealerTable.add(hs, computeDealerState(hs)); 
      hs = HandState(i, true, false);
      dealerTable.add(hs, computeDealerState(hs));
    }
    hs = HandState(3, true, false); // A + 2
    dealerTable.add(hs, computeDealerState(hs));    
    hs = HandState(2, true, false); // A + A
    dealerTable.add(hs, computeDealerState(hs));     
  }
  ...
  Prob<int> computeDealerState(HandState hs){
	...
	return dealerTable.get(HandState(hs.value + 10, false, false));
	...
	if (newVal <= 6)
	  result += prob.pdf[i] * dealerTable.get(HandState(newVal, true, false));
	else if (newVal <= 11)
	  result += prob.pdf[i] * dealerTable.get(HandState(newVal + 10, false, false)); 
	else
	  result += prob.pdf[i] * dealerTable.get(HandState(newVal, false, false));
      }
      return result;
    }
